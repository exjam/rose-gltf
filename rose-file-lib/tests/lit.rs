...
fn read_lit() {
    ...
    let first_obj = &lit.objects[0];
    let first_part = &first_obj.parts[0];
    let last_obj = &lit.objects[lit.objects.len() - 1];
    let last_part = &last_obj.parts[last_obj.parts.len() - 1];
...
fn write_lit() {
    ...
    let buffer = vec![0u8; lit_size as usize];
    ...
    let first_obj = &new_lit.objects[0];
    let first_part = &first_obj.parts[0];
    let last_obj = &new_lit.objects[new_lit.objects.len() - 1];
    let last_part = &last_obj.parts[last_obj.parts.len() - 1];
